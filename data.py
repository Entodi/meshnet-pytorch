...
class Subject:
    def __init__(self, path, input_filenames, 
        target_filename, subvolume_shape, preprocessing):
        self._path = path
    ...
    def load_volume(self):
        ...
        if self._target_filename:
            _target = np.array(nipy.load_image(
                self._path + self._target_filename).get_data(), dtype='uint8')
            ...
            assert _target.shape == self._volume_shape, \
                'Target shapes are not consistent with Input shapes for {}'.format(
                    self._path)
            self._target = torch.from_numpy(_target)
    ...
    def generate_nonoverlap_coordinates(self):
        ...
        z = generate_centered_nonoverlap_1d_grid(self._inputs[0], self._subvolume_shape[0])
        y = generate_centered_nonoverlap_1d_grid(self._inputs[1], self._subvolume_shape[1])
        x = generate_centered_nonoverlap_1d_grid(self._inputs[2], self._subvolume_shape[2])
...
class VolumetricDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        coords = None
        if self._isTest and coords_id < len(self._get_nonoverlap_coordinates()):
    ...
    def _load_volumes(self):
        ...
        for i in progressbar.progressbar(range(len(self._paths))):
            self._dataset[i] = Subject(self._paths[i], self._input_filenames, 
                self._target_filename, self._subvolume_shape, self._preprocessing)
